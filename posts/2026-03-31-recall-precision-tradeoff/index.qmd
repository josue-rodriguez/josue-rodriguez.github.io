...
For now, we're treating TPR & FPR as independent knob, but in practice they're coupled through the decision threshold (which we'll get to later).
...
For that, we need the Precision-Recall curve. We'll compute both curves from the same threshold sweep and show them side by side — ROC on the left, PR at different prevalences on the right.
...
If you're working in a low-prevalence setting — fraud detection, rare disease screening, content moderation — it's worth computing that ceiling before you start tuning hyperparameters. You might find that the precision you need simply isn't reachable at your base rate. And if the ceiling is below what your use case requires, the fix isn't a better model — it's changing the population you apply it to. Pre-filtering to raise effective prevalence, multi-stage classifiers that narrow the candidate pool, or accepting lower recall in exchange for usable precision are all ways to work within the constraint rather than against it.
