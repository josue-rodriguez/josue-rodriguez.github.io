...
description: |
 Even strong classifiers suffer low precision when the positive class is rare. I walk through some math to show what's actually achievable in this setting.
...
Here's what this model looks like with an arbitrary separation of $\mu = 5$. Any vertical line we draw becomes a decision threshold $t$. Everything to the right gets classified as positive and everything to the left gets classied as negative. Sliding $t$ left catches more true positives but also more false positives; sliding $t$ right does the opposite.
...
However, the ROC curve is completely invariant to prevalence! TPR and FPR are both conditional probabilities - conditioned on the true class - so the base rate never enters the picture. The ROC  curve will look identical regardless of whether 1% or 50% of transactions are fraudulent. That's useful for some purposes, but it means the ROC curve can't tell you how many of your flags will be false positives in practice.
...
pr_dfs = []
dot_rows = [] # Mark the 0.95 TPR / 0.05 FPR operating point for each prevalence
